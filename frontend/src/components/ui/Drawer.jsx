      ...
      <>
          ...
          <div className="flex flex-col h-full w-full">
            ...
            <nav className="flex flex-col p-4 gap-2">
              {visibleOptions.map((opt) => (
                <button
                  key={opt.label}
                  className="flex items-center gap-3 w-full p-3 rounded-lg hover:bg-green-50 text-gray-700 font-medium"
                >
                  {opt.icon}
                  <span>{opt.label}</span>
                </button>
        ...
        <div className="md:hidden fixed bottom-0 left-0 w-full bg-white shadow-t flex justify-around border-t z-50">
          {visibleOptions.map((opt) => (
            <button
              key={opt.label}
              className="flex flex-col items-center justify-center py-2 text-gray-700 hover:text-green-600"
            >
              {opt.icon}
