        ...
        <thead className="bg-gradient-to-r from-green-50 to-emerald-50 border-b border-gray-200">
          <tr>
            {columns.map((col, index) => (
              <th
                key={index}
                className="px-6 py-4 text-left font-bold text-gray-800 tracking-wide text-sm uppercase"
              >
                {col.header}
              </th>
            ))}
          ...
          {data.length === 0 ? (
            ...
            data.map((row, rowIndex) => (
              <tr
                ...
                className={`hover:bg-green-50/50 transition-all duration-200 ${rowIndex % 2 === 0 ? "bg-white" : "bg-gray-50/30"
                  }`}
              >
        ...
        <div className="px-6 py-3 bg-gray-50 border-t border-gray-200 rounded-b-xl">
          <div className="flex items-center justify-between text-sm text-gray-600">
            ...
            <div className="flex items-center gap-4">
              <button className="px-3 py-1 text-gray-600 hover:text-green-600 hover:bg-green-50 rounded-md transition">
                Previous
              </button>
              <button className="px-3 py-1 text-gray-600 hover:text-green-600 hover:bg-green-50 rounded-md transition">
                Next
